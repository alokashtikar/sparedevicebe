...
resource "aws_cloudfront_distribution" "backoffice_origin" {
  aliases = ["${var.bitz_backoffice_domain}"]
  origin {
    domain_name = "${aws_s3_bucket.backoffice.bucket_regional_domain_name}"
    origin_id   = "${var.s3_bo_bucket_name}"
 ...
 default_cache_behavior {
    ...
    target_origin_id = "${var.s3_bo_bucket_name}"
  ...
  viewer_certificate {
    ...
    acm_certificate_arn = "${var.bitz_cert_arn}"
  }
